...
import {getCategories, getMinMaxPrice} from '../../../services';
import {filterCategory, filterPrice} from '../../../actions'
...
class Filter extends Component {
    ...
    clickBrandHendle(event, category) {

        var index = category.indexOf(event.target.value);
        if (event.target.checked)
            category.push(event.target.value); // push in array checked value
        else
            category.splice(index, 1); // removed in array unchecked value
                                        ...
                                        {this.props.category.map((cat, index) => {
                                            return (
                                                <div className="custom-control custom-checkbox collection-filter-checkbox" key={index}>
                                                    <input type="checkbox" onClick={(e) => this.clickBrandHendle(e,filteredCategory)}
                                                            value={cat} 
                                                            // defaultChecked={filteredCategory.includes(cat)? true : false}
                                                              className="custom-control-input" id={cat} />
...
const mapStateToProps = state => ({
    category: getCategories(state.data.products),
